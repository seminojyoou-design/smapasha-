...
# クリップボードにファイルパスをセット（CF_HDROP形式）
def set_clipboard_file(filepath):
    """保存したファイルのパスをクリップボードにCF_HDROPで入れる（Win11同等の挙動にする）"""
    import struct
    CF_HDROP = 15
    GMEM_MOVEABLE = 0x0002
    GMEM_ZEROINIT = 0x0040

    # DROPFILES構造体 + ファイルパス（UTF-16） + ダブルNULL終端
    filepath_w = filepath.encode('utf-16-le') + b'\x00\x00'  # NULL終端
    # DROPFILES: pFiles(4) + pt.x(4) + pt.y(4) + fNC(4) + fWide(4) = 20バイト
    dropfiles = struct.pack('IIIii', 20, 0, 0, 0, 1)  # fWide=1 (Unicode)
    data = dropfiles + filepath_w + b'\x00\x00'  # リスト終端の追加NULL

    try:
        kernel32 = ctypes.windll.kernel32
        user32 = ctypes.windll.user32

        # 64bit対応: ポインタ/ハンドル系の戻り値型を明示
        ...
        if user32.OpenClipboard(0):
            ...
            if result:
                print(f"📋 クリップボードにファイルパスをセット: {filepath}")
                return True
            else:
                print("SetClipboardData失敗")
                kernel32.GlobalFree(hMem)
                return False
        else:
            kernel32.GlobalFree(hMem)
            print("OpenClipboard失敗")
            return False
    except Exception as e:
        print(f"クリップボードセットエラー: {e}")
        return False
        ...
        try:
            ...
            if img_data is not None:
                ...
                if img is not None:
                    ...
                    # 保存したファイルをCF_HDROPでクリップボードにセット（Win10でもペースト可能にする）
                    # 同じフォルダに貼り付けても衝突しないよう、TEMPにリネームしたコピーを作る
                    import shutil
                    import tempfile
                    temp_dir = os.path.join(tempfile.gettempdir(), "sumapasha_clipboard")
                    os.makedirs(temp_dir, exist_ok=True)
                    # 元ファイル名に(1)をつけてコピー → 保存先に貼っても衝突しない
                    base_name, ext = os.path.splitext(os.path.basename(filepath))
                    copy_name = f"{base_name}(1){ext}"
                    copy_path = os.path.join(temp_dir, copy_name)
                    shutil.copy2(filepath, copy_path)
                    print(f"📋 貼り付け用コピー作成: {copy_path}")
                    set_clipboard_file(copy_path)
                    return # 監視終了
